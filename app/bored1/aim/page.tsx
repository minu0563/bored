  ...
  return (
    <div className="flex flex-col justify-center items-center w-full relative overflow-hidden font-sans text-white" style={{ height: gameHeight }}>
      ...
      <div className="flex space-x-2 mb-4 z-10 relative">
        <button className="px-4 py-2 bg-gray-700 text-white rounded hover:bg-gray-600" onClick={() => router.push('/')}>
          back
        </button>
      ...
      {circles.map((c, i) => (
        <div
          ...
          style={{
            width: sizes[i],
            height: sizes[i],
            left: Number.isFinite(positions[i]?.x) ? positions[i].x : 0,
            top: Number.isFinite(positions[i]?.y) ? positions[i].y : 0,
